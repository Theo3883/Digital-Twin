...
using DigitalTwin.SyncClient.Interfaces;
using DigitalTwin.SyncClient.Clients;
using DigitalTwin.SyncClient.Providers;
...
public static class DependencyInjection
{
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    //  Embedded Engine  (local-only, HTTP sync clients, no cloud repos)
    // ═══════════════════════════════════════════════════════════════════════════

    /// <summary>
    /// Registers services for the embedded .NET engine (SwiftUI host).
    /// Uses local SQLite only, HTTP clients for cloud sync, no device providers
    /// (those are implemented in Swift and passed via bridge).
    /// </summary>
    public static IServiceCollection AddDigitalTwinForEmbeddedEngine(
        this IServiceCollection services,
        string localConnectionString,
        string baseApiUrl,
        Action<IServiceCollection>? registerNativeProviders = null)
    {
        ...
        // ── Infrastructure services (no cloud repos) ─────────────────────────
        services.AddSingleton<ITransientFailurePolicy, NpgsqlTransientFailurePolicy>();
        services.AddScoped<IDomainEventDispatcher, NoOpDomainEventDispatcher>();

        // ── HTTP Sync Clients ────────────────────────────────────────────────
        services.AddSingleton<IAuthTokenProvider, StaticAuthTokenProvider>();
        ...
        services.AddHttpClient<IPatientSyncApiClient, PatientSyncApiClient>(client =>
        {
            client.BaseAddress = new Uri(baseApiUrl);
            client.Timeout = TimeSpan.FromSeconds(30);
        });
        
        services.AddHttpClient<IVitalSignSyncApiClient, VitalSignSyncApiClient>(client =>
        {
            client.BaseAddress = new Uri(baseApiUrl);
            client.Timeout = TimeSpan.FromSeconds(30);
        });

        // ── Sync Drainers (using HTTP clients) ───────────────────────────────
        services.AddScoped<UserSyncDrainer>();
        services.AddScoped<PatientSyncDrainer>();
        services.AddScoped<VitalSignSyncDrainer>();
        ...
        services.AddSingleton<IHealthDataSyncService>(sp => new HealthDataSyncService(
            ...
            sp.GetRequiredService<ILogger<HealthDataSyncService>>(),
            sp.GetRequiredService<ITransientFailurePolicy>(),
            null)); // No cloud health service in embedded mode

        // ── Validators ───────────────────────────────────────────────────────
        services.AddValidation();

        // ── Native providers (implemented in Swift, registered via callback) ──
        registerNativeProviders?.Invoke(services);
